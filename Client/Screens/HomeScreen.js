...
export default function HomeScreen({ navigation }) {
  ...
  useEffect(() => {
    const fetchApiariesCount = async () => {
      try {
        if (currentUser) {
          const response = await axios.get('http://192.168.1.15:3000/api/apiary/getAllApiaries');
  ...
  const fetchHivesCount = async () => {
    try {
      if (currentUser) {
        const response = await axios.get('http://192.168.1.15:3000/api/hive/getAllHives');
        const hives = response.data.data;
  
        // Fetch all apiaries owned by the current user
        const apiariesResponse = await axios.get('http://192.168.1.15:3000/api/apiary/getAllApiaries');
